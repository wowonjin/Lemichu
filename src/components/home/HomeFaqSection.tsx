    ...
    <section className="bg-background" aria-labelledby="trust-heading">
      <div className="container py-12 md:py-16">
        ...
        <Stagger
          stagger={0.1}
          delay={0.08}
          className="mt-7 grid grid-cols-1 gap-3 md:mt-8 md:grid-cols-3 md:gap-4"
        >
          {trustCards.map((card) => (
            <StaggerItem key={card.id} variant="up" className="h-full">
            <Link
              href={card.href}
              className="group flex h-full flex-col rounded-md bg-[#F7F7F7] px-6 py-6 transition-colors duration-300 hover:bg-[#F0F0F0] dark:bg-muted dark:hover:bg-secondary md:min-h-[236px] md:px-7 md:py-7"
            >
