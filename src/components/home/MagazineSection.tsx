...
export function MagazineSection() {
  return (
    <section className="container py-10 md:py-14">
      <SectionHeader title="레미츄 매거진" moreHref="/magazine" />
        ...
        {magazineItems.map((item) => (
          <Link key={item.id} href={item.href} className="group block">
            <div className="relative aspect-[3/2] overflow-hidden rounded-2xl border border-border">
              ...
              <img src={getTemporaryImageUrl(item.seed)} alt={item.title} className="absolute inset-0 h-full w-full object-cover" />
