...
const sizeGuide = [
  { label: "PM", value: "28 x 15 x 34 cm / 약 250g" },
  { label: "MM", value: "32 x 17 x 37 cm / 약 310g" },
  { label: "GM", value: "34 x 20 x 40 cm / 약 350g" },
];
      ...
      <div className="container py-6 md:py-10">
          ...
          <aside className="lg:sticky lg:top-28 lg:self-start">
            ...
            <div className="border-b border-border py-5">
              ...
              <dl className="mt-4 divide-y divide-border text-sm">
                ...
                <InfoLine label="정품보장" value="가품 판정 시 결제금액 100% 보상" />
              </dl>
            </div>

            <div className="border-b border-border py-5">
              <OptionBlock title="사이즈">
                <div className="grid gap-2">
                  {sizeGuide.map((size) => (
                    <button
                      key={size.label}
                      type="button"
                      className={`flex min-h-12 items-center justify-between border px-4 text-left text-sm transition-colors hover:border-foreground ${
                        product.size === size.label || product.size?.includes(size.label)
                ...
                <div className="grid grid-cols-2 gap-2">
                  {colorOptions.map((color) => (
                    <button
                      key={color}
                      type="button"
                      className={`min-h-11 border px-3 text-left text-sm transition-colors hover:border-foreground ${
                        product.color === color
              ...
              <div className="mt-5 divide-y divide-border border-y border-border">
                {sizeGuide.map((size) => (
                  <div key={size.label} className="grid grid-cols-[120px_minmax(0,1fr)] py-3 text-sm">
                    ...
                    <dd className="text-muted-foreground">{size.value}</dd>
                  </div>
                ))}
              </div>
            </section>
        ...
        <section id="delivery" className="grid gap-10 border-b border-border py-10 lg:grid-cols-[180px_minmax(0,1fr)]">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            ...
            <PolicyCard
              title="정품 보장"
              description="가품 판정 시 결제금액 100% 보상"
              icon={ShieldCheck}
            />
            <PolicyCard
              title="문의"
