...
export async function POST(request: Request) {
  try {
    ...
    const payload = parseRelayDepositPayload(rawBody);
    ...
    const code = error instanceof Error ? error.message : "RELAY_DEPOSIT_FAILED";
    if (code === "INVALID_RELAY_PAYLOAD" || error instanceof SyntaxError) {
      return NextResponse.json(
        { ok: false, message: "입금 이벤트 형식이 올바르지 않습니다." },
        { status: 400 }
      );
    }
    if (
      code === "KB_RELAY_DEVICE_SECRET_NOT_CONFIGURED" ||
      code === "KB_RELAY_ACCOUNT_MASK_NOT_CONFIGURED"
    ) {
      return NextResponse.json(
    ...
    console.error("[bank-relay] deposit processing failed", code);
    return NextResponse.json(
      { ok: false, message: "입금 이벤트를 처리하지 못했습니다." },
      { status: 500 }
    );
  }
}
