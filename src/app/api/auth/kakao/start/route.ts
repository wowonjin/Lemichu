...
import { getKakaoLoginConfig, KAKAO_AUTHORIZE_URL } from "@/lib/kakao-login/config";
import { applyKakaoOAuthCookies } from "@/lib/kakao-login/cookies";
import { normalizeRedirectPath } from "@/lib/redirect";

export async function GET(request: Request) {
  try {
    const { clientId } = getKakaoLoginConfig();
    ...
    return applyKakaoOAuthCookies(response, state, redirectPath);
  } catch (error) {
    const message =
